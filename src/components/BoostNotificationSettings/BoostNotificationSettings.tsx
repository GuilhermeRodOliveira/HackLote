...
export default function BoostNotificationSettings() {
  ...
  // Função para salvar as preferências no backend
    ...
    try {
      ...
      const preferencesToSend = AVAILABLE_BOOST_OPTIONS.flatMap(gameOption =>
        gameOption.types.map(type => ({
          ...
          boostType: type,
          subscribed: userPreferences[`${gameOption.game}-${type}`] || false,
        }))
      );
      ...
      toast.error('Erro de rede ao salvar preferências.');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    ...
    <div className="max-w-3xl mx-auto py-4 px-4 sm:px-8">
